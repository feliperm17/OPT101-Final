...
A página web será acessível em http://<IP_DO_PC>:5000
"""
...
DB_PATH        = "alarme.db"
SENHA_CORRETA  = "1234"     # ALTERE a senha aqui
TEMPO_SENHA_S  = 15         # Segundos para digitar a senha após a porta abrir
...
def log_senha(senha: str, sucesso: bool):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute(
...
# ─── Serial: Comunicação com Arduino ─────────────────────────────────────────
def encontrar_arduino():
    ...
    for p in ports:
        ...
        if "arduino" in desc or "ch340" in desc or "cp210" in desc or "usb" in desc:
            return p.device
    # Se não achou automaticamente, retorna a primeira porta disponível
    if ports:
...
def acionar_alarme():
    """Aciona o alarme (envia comando para Arduino e notifica clientes web)."""
    with lock:
...
def thread_timeout_senha():
    """Thread que dispara alarme se a senha não for inserida a tempo."""
    inicio = estado["tempo_senha_inicio"]
    while True:
        time.sleep(0.5)
        with lock:
            if not estado["aguardando_senha"]:
                return  # Foi resolvido (senha correta ou alarme já ativado)
...
def processar_mensagem_arduino(linha: str):
    """Processa uma linha recebida do Arduino via Serial."""
    linha = linha.strip()
    if not linha:
        return
    ...
    if linha.startswith("PORTA_ABERTA:"):
        ...
        with lock:
            if not estado["porta_aberta"]:
                estado["porta_aberta"] = True
                estado["ultima_distancia"] = distancia
                if not estado["alarme_ativo"] and not estado["aguardando_senha"]:
        ...
        socketio.emit("estado_atualizado", get_estado_publico())
        socketio.emit("porta_aberta", {"distancia": distancia, "tempo_limite": TEMPO_SENHA_S})
...
def thread_serial_reader():
    """Thread dedicada à leitura da serial."""
    global arduino_serial
    while True:
...
def api_senha():
    ...
    log_senha(senha_digitada, sucesso)
...
def api_desarmar():
    """Endpoint para desarmar manualmente (uso do admin)."""
    data = request.get_json(force=True)
...
def api_simular():
    """Simula abertura/fechamento da porta (para testes sem hardware)."""
    data = request.get_json(force=True)
...
# (O frontend completo é servido em index.html separado, mas isso serve de fallback)
@app.route("/app")
def app_page():
...
def index():
    ...
    <html>
    <head>
        <title>Sistema de Alarme IoT</title>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
    </head>
    <body>
        <h1>Backend rodando!</h1>
        <p>Acesse <a href="/app">/app</a> para o painel completo.</p>
        <p>API: <a href="/api/estado">/api/estado</a></p>
    </body>
    </html>
    """)
...
if __name__ == "__main__":
    init_db()
    # Inicia thread de leitura serial em background
