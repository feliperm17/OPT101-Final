...
// ─── Aplicação ────────────────────────────────────────────────────────────────
#define DIST_THRESHOLD_CM 5  // acima de 5cm = porta aberta
...
#define DELAY_TX_MS     10
#define DELAY_RX_MS     10

RF24 radio(CE_PIN, CSN_PIN);
uint8_t seqNum = 0;
bool portaAbertaAntes = false;  // ← estado anterior da porta
...
float lerDistanciaCM() {
  ...
  if (duracao == 0) return 0; // sem eco = objeto muito longe ou erro
...
void loop() {
  ...
  // Só envia quando muda de estado (evita flood de mensagens)
