/*
 * PLACA 13 - GATEWAY
 * Hardware: Arduino Nano + nRF24L01 + USB Serial ao Backend
...
uint8_t seqNum = 0;
bool processandoAlarme = false; // ignora P30 enquanto alarme ativo
...
void processarMensagemP30(Payload &data) {
  if (data.dado2 == 1) {
    processandoAlarme = true;   // bloqueia novos pacotes da P30
    Serial.print(F("PORTA_ABERTA:"));
    Serial.println(data.dado1);
    // NÃO envia alarme para P7 aqui — backend decide
  } else {
    processandoAlarme = false;  // porta fechou, aceita P30 novamente
...
bool enviarParaP7(uint8_t tipo, uint8_t d1, uint8_t d2, uint8_t d3) {
  for (int tentativa = 0; tentativa < MAX_RETRIES; tentativa++) {
    ...
    // PASSO 2: Aguarda CTS — ignora pacotes de outras origens
    modoRX();
    ...
    while (millis() - t0 < TIMEOUT_CTS_MS) {
      if (radio.available()) {
        ...
        // Tolerante ao seq para evitar necessidade de reset
    ...
    // PASSO 3: DATA
    // Delay para P7 voltar ao modo RX após enviar CTS
    delay(50);
    ...
    // PASSO 4: Aguarda ACK — ignora pacotes de outras origens
    modoRX();
...
void verificarMensagensRF() {
  ...
  // Ignora P30 enquanto alarme em andamento
